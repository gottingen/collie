...
*/
    ...
    protected:
        small_vector_base(void *FirstEl, size_t Size)
    ...
    */
    ...
    class small_vector_template_common : public small_vector_base {
    ...
    protected:
        small_vector_template_common(size_t Size) : small_vector_base(&FirstEl, Size) {}

        void grow_pod(size_t MinSizeInBytes, size_t TSize) {
            small_vector_base::grow_pod(&FirstEl, MinSizeInBytes, TSize);
        }

        /// Return true if this is a small_vector which has not had dynamic
    ...
    */
    ...
    protected:
        small_vector_template_base(size_t Size) : small_vector_template_common<T>(Size) {}
    ...
    public:
        void push_back(const T &Elt) {
            if (COLLIE_UNLIKELY(this->EndX >= this->CapacityX))
                this->grow();
...
*/
    template<typename T, bool isPodLike>
    void small_vector_template_base<T, isPodLike>::grow(size_t MinSize) {
        size_t CurCapacity = this->capacity();
        size_t CurSize = this->size();
        // Always grow, even from zero.
        size_t NewCapacity = size_t(collie::detail::NextCapacity(CurCapacity + 2));
...
*/
    template<typename T>
    class small_vector_impl : public small_vector_template_base<T, IsPod<T>::value> {
        typedef small_vector_template_base<T, IsPod<T>::value> SuperClass;
    ...
    protected:
        ...
        explicit small_vector_impl(unsigned N)
                : small_vector_template_base<T, IsPod<T>::value>(N * sizeof(T)) {
        }
    ...
    template<typename T>
    small_vector_impl<T> &small_vector_impl<T>::
    operator=(const small_vector_impl<T> &RHS) {
    ...
    */
    ...
    struct small_vector_storage {
        ...
        @private
        */
        typename small_vector_template_common<T>::U InlineElts[N - 1];
    };
    ...
    */
    template<typename T>
    struct small_vector_storage<T, 1> {
    };
    ...
    */
    template<typename T>
    struct small_vector_storage<T, 0> {
    };
...
*/
    ...
    class small_vector : public small_vector_impl<T> {
        ...
        small_vector_storage<T, N> Storage;
    ...
    public:
        ...
        */
        ...
        //explicit small_vector(const collie::iterator_range<RangeTy> &R)
        //    : small_vector_impl<T>(N) {
        ...
        */
        const small_vector &operator=(small_vector &&RHS) {
            small_vector_impl<T>::operator=(::std::move(RHS));
            return *this;
        }
        ...
        */
        ...
        @brief replaces the contents with the contents of @c RHS using move semantics
         */
        const small_vector &operator=(small_vector_impl<T> &&RHS) {
            small_vector_impl<T>::operator=(::std::move(RHS));
            return *this;
        }
         ...
         */
        const small_vector &operator=(std::initializer_list<T> IL) {
            this->assign(IL);
            return *this;
        }
    };
    ...
    template<typename T, unsigned N>
    static inline size_t capacity_in_bytes(const small_vector<T, N> &X) {
...
namespace std {
    ...
    template<typename T>
    inline void
    swap(collie::small_vector_impl <T> &LHS, collie::small_vector_impl <T> &RHS) {
        LHS.swap(RHS);
    }
    ...
    template<typename T, unsigned N>
    inline void
    swap(collie::small_vector <T, N> &LHS, collie::small_vector <T, N> &RHS) {
        LHS.swap(RHS);
    }
}  // namespace std
